...
const bankAccountSchema = z
  ...
  .refine(
    ...
    {
      message: "Vui lòng nhập đầy đủ thông tin ngân hàng hoặc tải lên mã QR",
      path: ["accountNumber"],
    },
  )
...
export function BankAccountDialog({ open, onOpenChange, account }: BankAccountDialogProps) {
  ...
  const onSubmit = async (data: BankAccountFormData) => {
    try {
      ...
      toast.success(`Tài khoản ngân hàng đã được ${account ? "cập nhật" : "thêm"}`);

      onOpenChange(false)
    } catch (error) {
      toast.error("Lỗi", {
        description: `Đã có lỗi xảy ra khi ${account ? "cập nhật" : "thêm"} tài khoản ngân hàng`,
      })
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{account ? "Chỉnh sửa tài khoản ngân hàng" : "Thêm tài khoản ngân hàng"}</DialogTitle>
          <DialogDescription>Nhập thông tin tài khoản ngân hàng hoặc tải lên mã QR thanh toán.</DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <BankSelect onValueChange={field.onChange} value={field.value} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <DialogFooter>
              ...
              <Button disabled={!form.formState.isDirty || form.formState.isSubmitting} type="submit">{account ? "Cập nhật" : "Thêm"}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
