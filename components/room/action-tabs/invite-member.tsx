...
export function InviteMemberTab() {
    ...
    const inviteLink = room ? `${window.location.origin}/room/join/${room._id}` : '';
    ...
    const handleDownloadQR = () => {
        try {
            ...
            const titleHeight = title ? Math.floor(size * 0.30) : 0; // 30% chiều cao cho title
            ...
            const canvasHeight = size + titleHeight + (title ? titlePadding * 2 : 0);
            ...
            img.onload = function () {
                ...
                if (title) {
                    ...
                    while (ctx.measureText(title).width > size - (qrPadding * 2) && fontSize > 12) {
                        fontSize -= 2;
                        ctx.font = `bold ${fontSize}px Arial, sans-serif`;
                    }
    ...
    return (
        <div className="w-full lg:w-80 h-full bg-sidebar overflow-y-auto p-4 lg:p-6">
            ...
            <Card>
                <CardHeader>
                    <CardDescription>Gửi liên kết hoặc mã QR cho bạn bè để mời họ tham gia phòng.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Input onClick={(e) => e.currentTarget.select()} readOnly value={inviteLink} />
                    <Button className="mt-4 w-full" disabled={isCopied || !inviteLink} onClick={handleCopy}>
                        {isCopied ? "✅ Đã sao chép" : "Sao chép liên kết"}
                    </Button>
                    {/* Share button */}
                    <Button className="mt-4 w-full" variant="outline" disabled={!inviteLink} onClick={handleShare}>
                        Chia sẻ liên kết
                    </Button>

                    <div className="mt-6 flex justify-center">
                        <QRCodeSVG ref={qrcodeRef} width={128} height={128} value={inviteLink} />
                    </div>

                    {/* Download QR button */}
                    <Button className="mt-4 w-full" disabled={!inviteLink} onClick={handleDownloadQR}>
                        Tải mã QR
                    </Button>
                </CardContent>
            </Card>
