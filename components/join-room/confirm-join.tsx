...
export function ConfirmJoinRoomPage({ room }: { room: IRoom }) {
    ...
    return <div className="min-h-[80vh] bg-background flex items-center justify-center">
        <Card className="w-full max-w-md mx-auto shadow-lg">
            <CardHeader>
                <CardTitle>Bạn có muốn tham gia phòng <b>{room.name}</b> không?</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col gap-4 *:flex *:items-center *:gap-1 *:text-muted-foreground *:text-sm">
                ...
                <div>
                    ...
                    <span className="">Thành viên: {room.members.length}/{room.maxMembers}</span>
                </div>
                <div>
                    <Clock className="w-4 h-4" />
                    <span className="">Tạo vào: {formatDate(new Date(room.createdAt))}</span>
                </div>
            ...
            <CardFooter className="flex gap-2 justify-end">
                ...
                <Button disabled={isJoining} onClick={handleJoinRoom}>
                    Tham gia
                    <span>{isJoining && <Loader2 className="animate-spin ml-2" />}</span>
                </Button>
            </CardFooter>
        </Card>
    </div>
