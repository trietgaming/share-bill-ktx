...
export const invoiceSchema = new Schema<IInvoice>(
    {
        ...
        monthApplied: {
            ...
            match: [
                /^\d{4}-(0[1-9]|1[0-2])$/,
                "Month applied must be in YYYY-MM format",
            ],
            ...
            validate: {
                ...
                message:
                    "Month applied is required for walec and roomCost type invoice",
            },
        },
        ...
        payTo: {
            ...
            maxLength: [
                1024,
                "Pay to info can not be more than 1024 characters",
            ],
            validate: {
...
invoiceSchema.virtual("remainingAmount").get(function (this: IInvoice) {
    const paidAmount =
        this.payInfo?.reduce((sum, pay) => sum + pay.amount, 0) || 0;
...
invoiceSchema.index({ roomId: 1, applyTo: 1, status: 1 });
